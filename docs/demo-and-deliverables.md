...
| 1:00–2:30 | **Teach loop:** learner says "I don't get attention" → retrieves span → explains with an analogy → check-question → learner half-right → **re-explains differently, same citation** → it clicks. Show the **trace panel** side-by-side. | The agenticity proof |
...
| 4:00–4:45 | **Standout move:** same concept, two personas back-to-back — PM analogy, then engineer pseudocode. | Creativity + personalization |
| 4:45–5:00 | "Next: quiz, then interview — same engine." + architecture thumbnail. | Forward momentum |
...
3. **Hard trade-offs** (1 p) — why I cut Mem0 / voice / explicit LangGraph; why `create_agent` won for a
   Thursday ship. (show the cut list)
...
7. **What I learned** (½ p) — e.g. "if I did Week 3 again, I'd build the trace viewer on day 1."
...
| # | Move | Effort | Payoff |
|---|---|---|---|
| S1 | Two-persona demo back-to-back (same concept, two styles) | ~10 min prompt change | Creativity + "actually adapts" |
...
| Datasets | ⚠️ permission pending | Confirm CohortBrain in writing (or Plan B, `roadmap.md`) |
